...
import { deliverMessagePush } from "@/lib/message-push";
...
export async function startConversation(formData: FormData) {
  ...
  if (!postId) {
    finish("/casos", "La publicación no es válida.", true);
  }
...
if (messageId) {
  console.log("MESSAGE CREATED FOR PUSH:", messageId);

  const result = await deliverMessagePush(String(messageId));
...
export async function archiveConversation(formData: FormData) {
  ...
  if (error) {
    finish(
      `/conversaciones/${conversationId}`,
      error.message || "No se pudo archivar la conversación.",
      true
    );
  }
...
export async function setConversationBlock(formData: FormData) {
  ...
  if (error) {
    finish(
      `/conversaciones/${conversationId}`,
      error.message || "No se pudo cambiar el bloqueo.",
      true
    );
  }
  ...
  if (!blocked) {
    ...
    if (unarchiveError) {
      finish(
        ...
        "El usuario fue desbloqueado, pero no se pudo reabrir la conversación.",
        true
      );
    }
  }
  ...
  finish(
    `/conversaciones/${conversationId}`,
    blocked
      ? "Usuario bloqueado. La conversación quedó archivada."
...
export async function reportConversationMessage(formData: FormData) {
  ...
  if (error) {
    finish(
      `/conversaciones/${conversationId}`,
      error.message || "No se pudo enviar la denuncia.",
      true
    );
  }
  ...
  finish(
    `/conversaciones/${conversationId}`,
    "La denuncia fue enviada a administración."
  );
}